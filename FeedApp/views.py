...
def profile(request):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('FeedApp:profile') # can change to redirect user to somewhere else
...
#Question for ProfB: Why does my post show up on My Feed and my Friends Feed?
@login_required
def friends(request):
